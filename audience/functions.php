...
function update($table, $where, $data, $asIs = "", $con = "") {
 ...
 // $data = _update($data);
   ...
   echo $q = "UPDATE `" . $table . "` SET " . implode(", ", $sets) . " WHERE $where";
    $data = "";
...
function delete($table, $whr, $con = "") {
   ...
   echo $q = "delete from {$table} where $whr";
...
function clean_array($arr){
    ...
    foreach($arr as $_arr){
        if(trim($_arr)!=''){
            array_push($res, trim($_arr));
        }
    }

    return $res;
}
function pagination($total, $offset, $limit, $page, $link=''){
